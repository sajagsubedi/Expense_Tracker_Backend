  ...
  Query: {
    transactions: async (_, __, context) => {
      try {
        ...
        throw new Error(err.message || "Error on fetching transactions");
      }
    },
    ...
    categoryStatistics: async (_, __, context) => {
      try {
        ...
        return Object.entries(categoryTransactions).map(
          ([category, amount]) => ({ category, amount })
        );
      } catch (err) {
        throw new Error(err.message || "Error on fetching category statistics");
      }
    },
  },
  Mutation: {
